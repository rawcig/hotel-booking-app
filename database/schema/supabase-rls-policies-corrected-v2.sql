-- Corrected Supabase RLS Policies Script (casts, auth.uid() wrapping, RLS enabled where needed)

-- Ensure RLS enabled where missing
...
-- Indexes for policy subqueries (improve performance)
CREATE INDEX IF NOT EXISTS idx_reservation_bookings_booking_id ON public.reservation_bookings(booking_id);
CREATE INDEX IF NOT EXISTS idx_reservation_bookings_reservation_id ON public.reservation_bookings(reservation_id);
...
CREATE POLICY "Only admins can insert hotels"
  ...
  WITH CHECK (
    EXISTS (
      SELECT 1 FROM public.users WHERE id = (SELECT auth.uid()) AND role_id = 1
    )
  );
...
CREATE POLICY "Users can view their own reservations through bookings"
  ...
  USING (
    id IN (
      ...
      WHERE b.user_id = (SELECT auth.uid())::text
    )
    OR EXISTS (
      SELECT 1 FROM public.users WHERE id = (SELECT auth.uid()) AND role_id IN (1,3,4)
    )
  );
...
CREATE POLICY "Reservation bookings are viewable by linked users or staff"
  ...
  USING (
    ...
    AND (
      (SELECT user_id FROM public.bookings WHERE id = booking_id) = (SELECT auth.uid())::text
      OR EXISTS (SELECT 1 FROM public.users WHERE id = (SELECT auth.uid()) AND role_id IN (1,3,4))
    )
  );
...
CREATE POLICY "Only admins can update reservation bookings"
  ...
  USING (
    EXISTS (SELECT 1 FROM public.users WHERE id = (SELECT auth.uid()) AND role_id = 1)
  );
...
CREATE POLICY "Staff can insert payments"
  ...
  WITH CHECK (
    EXISTS (SELECT 1 FROM public.users WHERE id = (SELECT auth.uid()) AND role_id IN (1,3,4))
  );
