-- Corrected Supabase RLS Policies Script (Proper UUID handling)
...
CREATE POLICY "Users can view their own reservations through bookings"
  ...
  USING (
    ...
    OR EXISTS (
      SELECT 1 FROM public.users WHERE id = auth.uid() AND role_id IN (1,3,4)
    )
  );
...
CREATE POLICY "Reservation bookings are viewable by linked users or staff"
  ...
  USING (
    ...
    AND (
      ...
      OR EXISTS (SELECT 1 FROM public.users WHERE id = auth.uid() AND role_id IN (1,3,4))
    )
  );
...
CREATE POLICY "Only admins can update reservation bookings"
  ...
  USING (
    EXISTS (SELECT 1 FROM public.users WHERE id = auth.uid() AND role_id = 1)
  );
...
CREATE POLICY "Staff can insert payments"
  ...
  WITH CHECK (
    EXISTS (SELECT 1 FROM public.users WHERE id = auth.uid() AND role_id IN (1,3,4))
  );
